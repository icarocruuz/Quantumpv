...
def show_main_screen(page: ft.Page, user_session: UserSession):
    ...
    if strategy_dropdown: strategy_dropdown.value = str(getattr(user_session, 'strategy_var', 1)) # Assumindo que o valor é string
    if asset_dropdown: asset_dropdown.value = getattr(user_session, 'asset_var', None) # Assumindo que o valor é a key
...
def start_strategy(page, user_session):
    ...
    if strategy_function:
        ...
        print(f"[start_strategy] Estratégia selecionada inválida: {selected_strategy}") # DEBUG
        log_console(page, f"Erro: Estratégia {selected_strategy} não encontrada.", level="error")
        enable_buttons(page) # Reabilitar botões se a estratégia não for encontrada
        # Reverter status se necessário
        status_label_main = page.ui.get('status_label')
        if status_label_main and status_label_main.value == "🚀 Operando...":
            status_label_main.value = "Não Conectado" # Ou o status anterior
            status_label_main.color = quantum_theme["error"]
        page.update()
